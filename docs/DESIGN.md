...
- **Space is the home screen; chips are the reward.** Home leads with what chippytea has saved — space freed for good by reviewed cleanups, measured after each one, never estimated and never counting Trash — then the room left on the disk and the next few cleanup opportunities. The chips sit underneath in their own strip: one chip per 100 MB saved, heaped in an open paper wrap beside a hand-lettered count, "958 chips". No other denomination: nothing is counted but chips. Deeper tools are one tab away. The word "stash" does not appear in the interface. In copy the wrap is always **the paper** ("chips in the paper") — "wrap" is the drawn object only; as a noun in a sentence it reads like a sandwich, not a chippy.
...
3. **Hand-lettered numerals.** The balance is not a font: digits 0–9 are drawn stroke paths (28 pt in the chips strip, fixed advance) with gold hatch fill and ink outlines, and the unit — *chips* — is written after the number in the same lettering, small and in plain ink, sitting on the digits' baseline. A digit that changes boils for a few frames. Only the balance earns this; body text stays a system face for legibility. The wordmark letters (`chippytea`, lowercase centre-line strokes on the same 92-unit box) are drawn the same way, stroked in plain ink.
...
5. **The battered fish.** The shop sign: one closed ink outline over scribble-hatched gold batter, an eye, a gill, tail creases, a wisp of steam. Full colour in the masthead and About; monochrome ink template in the menu bar.
